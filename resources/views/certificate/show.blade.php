...
<body class="bg-slate-100 py-8">
    ...
    <div class="no-print max-w-4xl mx-auto mb-6 flex items-center justify-between px-4">
        <a href="{{ route('student.dashboard') }}" class="text-sm text-slate-600 hover:text-indigo-600">
            <i class="fa-solid fa-arrow-left mr-1"></i> Back to My Courses
        </a>
    ...
    <div class="max-w-4xl mx-auto bg-white shadow-xl">
        <div class="m-4 border-4 border-indigo-900 p-10 text-center relative">
            ...
            <div class="font-script text-6xl text-indigo-900 mb-4">{{ $user->name }}</div>

            <p class="text-sm text-slate-600 max-w-xl mx-auto mb-8">
                for successfully completing the course
                <span class="font-semibold text-slate-800">"{{ $course->title }}"</span>
                @if ($course->duration) <br>Duration: {{ $course->duration }} @endif
            </p>
