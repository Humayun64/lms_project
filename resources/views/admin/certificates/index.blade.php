...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Online certificates are auto-issued; offline ones are issued manually</p>
        </div>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-slate-800">
                    @forelse ($certificates as $cert)
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-800/50">
                            ...
                            <td class="px-6 py-4 text-slate-800 dark:text-white">{{ $cert->display_name }}</td>
                            <td class="px-6 py-4 text-slate-500 dark:text-slate-400">{{ $cert->course->title ?? '—' }}</td>
                            <td class="px-6 py-4">
                                @if ($cert->source === 'offline')
                                    <span class="text-xs px-2 py-1 rounded-full bg-orange-100 text-orange-700">Offline (manual)</span>
                                @else
                                    <span class="text-xs px-2 py-1 rounded-full bg-emerald-100 text-emerald-700">Online (auto)</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-slate-500 dark:text-slate-400">{{ $cert->issued_at?->format('d M Y') }}</td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-3">
                                    <a href="{{ route('admin.certificates.print', $cert) }}" target="_blank" class="text-indigo-600 hover:underline text-xs">
                                        <i class="fa-solid fa-print mr-0.5"></i> Print
                                    </a>
                                    <a href="{{ url('/verify/' . $cert->certificate_number) }}" target="_blank" class="text-slate-500 hover:underline text-xs">
                                        Verify <i class="fa-solid fa-arrow-up-right-from-square ml-0.5"></i>
                                    </a>
                                </div>
                            </td>
                        </tr>
