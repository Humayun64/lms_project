...
@section('content')
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-2xl font-semibold text-slate-800 dark:text-white">Edit User</h1>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 p-6 max-w-2xl">
        <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-5">
            ...
            <div>
                <label class="{{ $label }}">Password <span class="text-xs text-slate-400">(leave blank to keep current)</span></label>
            ...
            <div class="flex items-center gap-3 pt-2">
                <button type="submit" class="bg-indigo-600 text-white px-5 py-2.5 rounded-lg hover:bg-indigo-700 transition text-sm font-medium">Update User</button>
