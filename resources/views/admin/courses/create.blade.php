...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.courses.store') }}" enctype="multipart/form-data" class="space-y-5">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" class="{{ $input }}" placeholder="e.g. Professional Digital Marketing" required>
                ...
                <div class="md:col-span-2">
                    ...
                    <p class="text-xs text-slate-400 mt-1">Optional. JPG/PNG, max 2MB.</p>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 p-6">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <textarea name="short_description" rows="2" class="{{ $input }}" placeholder="One-line summary shown on cards">{{ old('short_description') }}</textarea>
                </div>
                <div>
                    <label class="{{ $label }}">Full description</label>
                    <textarea name="description" rows="4" class="{{ $input }}" placeholder="Detailed course description">{{ old('description') }}</textarea>
                </div>
                <div>
                    <label class="{{ $label }}">Outcome / career paths</label>
                    <textarea name="outcome" rows="2" class="{{ $input }}" placeholder="e.g. Digital Marketing Executive, SEO Specialist, Freelancer">{{ old('outcome') }}</textarea>
                </div>
                <div>
                    <label class="{{ $label }}">Final / capstone project</label>
                    <textarea name="final_project" rows="2" class="{{ $input }}" placeholder="Optional">{{ old('final_project') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="number" step="0.01" name="price" value="{{ old('price') }}" class="{{ $input }}" placeholder="Leave empty for now">
                </div>
                <div>
                    <label class="{{ $label }}">Sale price</label>
                    <input type="number" step="0.01" name="sale_price" value="{{ old('sale_price') }}" class="{{ $input }}" placeholder="Optional">
                </div>
                <div>
        ...
        <div class="flex items-center gap-3">
            <button type="submit" class="bg-indigo-600 text-white px-5 py-2.5 rounded-lg hover:bg-indigo-700 transition text-sm font-medium">Save Course</button>
